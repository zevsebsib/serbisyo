...
class _AdminRequestsScreenState extends State<AdminRequestsScreen> {
    ...
    try {
      ...
      for (final userId in userIds) {
        try {
          ...
          if (userDoc.exists) {
            userNames[userId] = userDoc.data()?['fullName'] ?? 'Unknown';
          }
        } catch (_) {}
      }
      ...
      _allRequests = reqSnap.docs.map((d) {
        ...
        return {
          ...
          'userId':          userId,
          'citizenName':     userNames[userId] ?? 'Unknown Citizen',
  ...
  void _applyFilters() {
    ...
    if (_departmentFilter != 'all') {
      result = result.where((r) => r['department'] == _departmentFilter).toList();
    }
    if (_dateRange != null) {
    ...
    if (_searchQuery.isNotEmpty) {
      ...
      result = result.where((r) =>
          ...
          r['serviceName'].toString().toLowerCase().contains(q)).toList();
    }
    result.sort((a, b) {
            ...
            child: SizedBox(
              ...
              child: TextField(
                ...
                decoration: InputDecoration(
                  hintText: 'Search by tracking ID or citizen name...',
          ...
          if (_departments.isNotEmpty) ...[
            Expanded(
              ...
              child: _buildDropdown(
                ...
                items: {
                  'all': 'All Departments',
                  ..._departments.asMap().map((_, d) => MapEntry(d, d)),
                },
                onChanged: (v) {
                  _departmentFilter = v!;
                  _applyFilters();
  ...
  Widget _buildStatsRow() {
    // Using List of Maps instead of records (Dart <3.0 compatible)
    final stats = <Map<String, dynamic>>[
      ...
      },
      {
        'key':   'pending',
        'label': 'Pending',
        'value': _allRequests.where((r) => r['status'] == 'pending').length,
        'color': const Color(0xFFF59E0B),
      },
      {
        'key':   'in_progress',
        'label': 'In Progress',
        'value': _allRequests.where((r) => r['status'] == 'in_progress').length,
        'color': const Color(0xFF3B82F6),
      },
      {
        'key':   'completed',
        'label': 'Completed',
        'value': _allRequests.where((r) => r['status'] == 'completed').length,
        'color': const Color(0xFF10B981),
      },
      {
        'key':   'rejected',
        'label': 'Rejected',
        'value': _allRequests.where((r) => r['status'] == 'rejected').length,
      ...
      child: Padding(
        ...
        child: Row(
          children: [
            ...
            Expanded(
              flex: 2,
              child: Text(r['trackingId'] as String,
                  overflow: TextOverflow.ellipsis,
                  style: GoogleFonts.inter(
                    fontSize: 12,
                    fontWeight: FontWeight.w700,
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: () async {
                ...
                Navigator.pop(ctx);
                await _updateStatus(
                    r['id'] as String, selectedStatus, note);
              },
              style: ElevatedButton.styleFrom(
                backgroundColor: AppColors.primary,
                elevation: 0,
